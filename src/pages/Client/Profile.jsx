  ...
  const removeAccountType = async (accountType) => {
    ...
    try {
      ...
      if (activeProfileType === accountType) {
        const remainingTypes = availableAccountTypes.filter(
...
const handleProfilePictureChange = async (e) => {
  ...
  const handleEducationSave = async (eduData) => {
    try {
      if (editingItem) {
        const updatedEdu = await apiCall(
          `/profile/education/${editingItem.id}/`,
          "PATCH",
          eduData
        );
        setEducation((prev) =>
          prev.map((edu) => (edu.id === editingItem.id ? updatedEdu : edu))
        );
      } else {
  ...
  const handleExperienceSave = async (expData) => {
    try {
      if (editingItem) {
        const updatedExp = await apiCall(
          `/profile/experience/${editingItem.id}/`,
          "PATCH",
          expData
        );
        setExperience((prev) =>
          prev.map((exp) => (exp.id === editingItem.id ? updatedExp : exp))
        );
      } else {
  ...
  const handleCertificationSave = async (certData) => {
    try {
      if (editingItem) {
        const updatedCert = await apiCall(
          `/profile/certifications/${editingItem.id}/`,
          "PATCH",
          certData
        );
        setCertifications((prev) =>
          prev.map((cert) => (cert.id === editingItem.id ? updatedCert : cert))
        );
      } else {
        const newCert = await apiCall(
  ...
  const handlePortfolioSave = async (portfolioData) => {
    try {
      if (editingItem) {
        const updatedPortfolio = await apiCall(
          `/profile/portfolio/${editingItem.id}/`,
          "PATCH",
          portfolioData
        );
        setPortfolio((prev) =>
          prev.map((item) =>
            item.id === editingItem.id ? updatedPortfolio : item
          )
        );
      } else {
        const newPortfolio = await apiCall(
          "/profile/portfolio/",
          "POST",
          portfolioData
        );
  ...
  const handleArrayRemove = (profileType, field, index) => {
    setUser((prev) => ({
      ...
      [profileType]: {
        ...prev[profileType],
        [field]:
          prev[profileType]?.[field]?.filter((_, i) => i !== index) || [],
      },
    }));
  };
      ...
      if (
        ...
        updateData.freelancer_profile = {
          ...
          availability_hours_per_week:
            user.freelancer_profile.availability_hours_per_week,
        };
      }
  ...
  const AccountTypeModal = () => {
    ...
    const availableToAdd = allAccountTypes.filter(
      (type) => !availableAccountTypes.includes(type)
    );
      ...
      {profileVisibility.isPublic ? (
        <div className="space-y-4">
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <p className="text-sm text-green-700 mb-3">
              Other users can view your profile and contact you for
              opportunities.
            </p>
            <div className="space-y-2">
        ...
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <p className="text-sm text-yellow-700 mb-3">
            Your profile is not visible to other users. Enable public profile to
                ...
                {[
                  {
                    key: "showEmail",
                    label: "Email Address",
                    desc: "Show your email to other users",
                  },
                  {
                    key: "showPhone",
                    label: "Phone Number",
                    desc: "Show your phone number to other users",
                  },
                  {
                    key: "showLocation",
                    label: "Location",
                    desc: "Show your city and country",
                  },
                  {
                    key: "showExperience",
                    label: "Work Experience",
                    desc: "Show your work history",
                  },
                  {
                    key: "showEducation",
                    label: "Education",
                    desc: "Show your educational background",
                  },
                  {
                    key: "showPortfolio",
                    label: "Portfolio",
                    desc: "Show your portfolio items",
                  },
                ].map((item) => (
                  <div
                    key={item.key}
  ...
  const PortfolioModal = () => {
    ...
    const addTechnology = () => {
      if (
        techInput.trim() &&
        !formData.technologies_used.includes(techInput.trim())
      ) {
        setFormData({
          ...formData,
          ...
          <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleBasicInputChange("first_name", e.target.value)
                    }
                    disabled={!isEditing}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleBasicInputChange("last_name", e.target.value)
                    }
                    disabled={!isEditing}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50"
                  />
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleBasicInputChange("phone_number", e.target.value)
                      }
                      disabled={!isEditing}
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    value={user.bio || ""}
                    onChange={(e) =>
                      handleBasicInputChange("bio", e.target.value)
                    }
                    disabled={!isEditing}
                    rows={4}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50"
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-8">
              ...
              <div className="mt-6">
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {(user.professional_profile?.languages_spoken || []).map(
                      (language, index) => (
                        <span
                          key={index}
          ...
          availableAccountTypes.includes("freelancer") && (
            <>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6 mb-8">
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {(user.freelancer_profile?.skills || []).map(
                      (skill, index) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800"
